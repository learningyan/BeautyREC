...
dataroot = 'mtdataset/images'
dirmap = 'mtdataset/parsing'
n_componets = 3
img_size = 256
...
class MTDataset():
    def __init__(self, phase):
        ...
        self.dir_seg = dirmap  # parsing maps
        self.n_componets = n_componets
        self.makeup_names = []
        self.non_makeup_names = []
        if self.phase == 'train':
            self.makeup_names = [name.strip() for name in
                                 open(os.path.join('mtdataset', 'makeup.txt'), "rt").readlines()]
            self.non_makeup_names = [name.strip() for name in
                                     open(os.path.join('mtdataset', 'nomakeup.txt'), "rt").readlines()]
        if self.phase == 'test':

            self.makeup_names = [name.strip() for name in
                                 open(os.path.join('mtdataset', 'makeup_test.txt'), "rt").readlines()]
            self.non_makeup_names = [name.strip() for name in
                                     open(os.path.join('mtdataset', 'nomakeup_test.txt'), "rt").readlines()]
            # self.non_makeup_names = [name.strip() for name in
            #                 open(os.path.join('mtdataset', 'makeup_test.txt'), "rt").readlines()]
            # import random
            # random.seed(66)
