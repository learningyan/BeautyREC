...
class StyleModulation(nn.Module):
    def __init__(self, dim):
        ...
        self.sen_lip = SELayer(dim)
        self.sa_lip = SpatialGate()

        self.sen_skin = SELayer(dim)
        self.sa_skin = SpatialGate()

        self.sen_eye = SELayer(dim)
        self.sa_eye = SpatialGate()  

         

    def forward(self, style, content, mask):    
        _,c_c,h_c, w_c = content.shape
        _,c_s,h,w = style.shape
        _,_,h_m,w_m = mask.shape

        if h!=h_c or w!=w_c:
            style = F.interpolate(style, size=[h_c, w_c])
            # stylex = F.interpolate(stylex, size=[h_c, w_c])

        if h_m!=h_c or w_m!=w_c:
            mask = F.interpolate(mask, size=[h_c, w_c])

        style_lip  = style[:,:c_s//3,:,:]
        style_skin = style[:,c_s//3:c_s//3*2,:,:]
        style_eye = style[:,c_s//3*2:,:,:]

        mask_lip = mask[:,0,:,:]
        mask_lip = torch.unsqueeze(mask_lip, 1).repeat(1, c_c, 1, 1)

        mask_skin = mask[:,1,:,:]
        mask_skin = torch.unsqueeze(mask_skin, 1).repeat(1, c_c, 1, 1)

        mask_eye = mask[:,2,:,:]
        mask_eye = torch.unsqueeze(mask_eye, 1).repeat(1, c_c, 1, 1)

        content_global_lip = self.sen_lip(style_lip, content)
        content_local_lip = self.sa_lip(style_lip, content_global_lip)
        content_lip_after = content*(1-mask_lip)+content_local_lip*(mask_lip)

        content_global_skin = self.sen_skin(style_skin, content_lip_after)
        content_local_skin = self.sa_skin(style_skin, content_global_skin)
        content_skin_after = content_lip_after*(1-mask_skin)+content_local_skin*(mask_skin)

        content_global_eye = self.sen_eye(style_eye, content_skin_after)
        content_local_eye = self.sa_eye(style_eye, content_global_eye)
        content_eye_after = content_skin_after*(1-mask_eye)+content_local_eye*(mask_eye)

        return content_eye_after
...
class DMTN(nn.Module):
    def __init__(self, params):
        ...
        self.style_encoder = StyleEncoder(n_downsample, 3, dim, style_dim, norm='none', activ=activ, pad_type=pad_type)
        self.style_encoder_global = StyleEncoder(n_downsample, 3, dim, style_dim, norm='none', activ=activ, pad_type=pad_type)
        self.modulate1 = StyleModulation(dim)
        self.modulate2 = StyleModulation(dim)
        self.modulate3 = StyleModulation(dim)

        self.attention1 = TransformerDecoderUnit(feat_dim = dim)


        # reconstruc decoder
        self.res = ResBlocks(n_res, dim*2, norm='in', activation=activ, pad_type=pad_type)
        self.up = nn.Upsample(scale_factor=2)
        self.up1 = nn.Upsample(scale_factor=2)
        self.upconv1 = Conv2dBlock(dim*3, dim, 3, 1, 1, norm='ln', activation=activ, pad_type=pad_type)
    ...
    def forward(self, x, y, y_seg, x_seg): ### x: no-makeup y: make up  y_seg: make up mask x_seg: np-makeup mask
        content1, content2, content3, style3 = self.encode(x, y, y_seg)
        style_global = self.style_encoder_global(y)

        modulation1 = self.modulate1(style3, content1, x_seg)
        modulation2 = self.modulate2(style3, content2, x_seg)
        modulation3 = self.modulate3(style3, content3, x_seg)


        modulation3_global = self.attention1(style_global,content3,content3)



        modulation = torch.cat([modulation3_global, modulation3],dim=1)
        recon3 = self.res(modulation)
        recon23 = torch.cat([ modulation2, recon3], dim=1)
        recon23 = self.upconv1(self.up1(recon23))  
        recon12 = torch.cat([ modulation1, recon23], dim=1)
        recon12 = self.upconv2(self.up2(recon12))  
        images_recon = self.out(recon12)
        
        return images_recon

    def encode(self, x, y, y_seg):
        # encode an image to its content and style codes
        import pdb
        for i in range(0, y_seg.size(1)):
            yi = y_seg[:, i, :, :]

            yi = torch.unsqueeze(yi, 1).repeat(1, y.size(1), 1, 1)
            yi = y.mul(yi)

            if i==0:
                style = self.style_encoder(yi)
            else:
                style = torch.cat([style, self.style_encoder(yi)], dim=1)

        content1, content2, content3 = self.content_encoder(x)
        return content1, content2, content3, style
